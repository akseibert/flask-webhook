...
- For activities:
  ...
  - Map phrases like "Work was done at [location]" to "site_name" and "activities" (e.g., {"site_name": "East Wing", "activities": ["Work was done"]}).
- For site_name:
...
- For roles:
  - Recognize "add [name] as [role]", "supervisors were [name]", "[name] was handling [role]", or inputs like "Roles supervisor" to assign "Supervisor" to the user.
  - Do not map weather-related or activity-related phrases to roles (e.g., "weather was cloudy" or "work was done" are not roles).
- For company:
  - Recognize "Company: [name]", "Companies: [name]", or "by [company]".
- For tools and service:
  - Recognize "Tool: [item]", "Tools: [item]", "Service: [task]", "Services: [task]", or phrases like "Tools used included [item]".
  - "Tools: none" or "Services: none" clears the respective list.
- For time:
...
4. Input: "Services: abc"
   Output: {"service": [{"task": "abc"}]}
5. Input: "Tools: none"
   Output: {"tools": []}
6. Input: "Roles supervisor"
   Output: {"people": ["User"], "roles": [{"name": "User", "role": "Supervisor"}]}
7. Input: "Work was done at the East Wing."
   Output: {"site_name": "East Wing", "activities": ["Work was done"]}
8. Input: "Good morning at the Central Plaza site, segment 5. Companies BuildRight AG and Electric Flow GmbH, supervisors Anna Keller and Marcus Schmidt. Weather was cloudy."
   Output: {
...
FIELD_PATTERNS = {
    ...
    "supervisor": r'^(?:supervisors\s*(?:were|are)\s+|i\s+was\s+supervising|i\s+am\s+supervising|i\s+supervised|roles?\s*[:,]?\s*supervisor\s*$)([^,]+?)?(?=(?:\s*,\s*(?:site|segment|category|company|people|service|tool|activity|issue|time|weather|impression|comments)\s*:)|$)',
    ...
    "clear": r'^(issues|activities|comments|tools|service)\s*[:,]?\s*none$',
    ...
    for field, pattern in FIELD_PATTERNS.items():
        ...
        if match:
            ...
            if field == "people":
                ...
                if match.group(1):  # Names provided
                ...
                else:  # "Roles supervisor" or similar
                    result["people"] = ["User"]
                ...
                result[field_name] = [] if field_name in ["issues", "activities", "tools", "service"] else ""
    ...
    try:
        ...
        if "roles" in data:
            for role in data["roles"]:
                if isinstance(role, dict) and "name" in role and role from xAI, role["name"] not in data.get("people", []):
        ...
        if normalized_text in ("export pdf", "/export pdf"):
            ...
            if pdf_buffer:
                if send_pdf_to_user(chat_id, pdf_buffer):
                    ...
                    send_telegram_response = requests.post(url, files=files, data=data)
                    response.raise_for_status()
                    logger.info({"event": "pdf_sent", "chat_id": chat_id})
                    return True
                except Exception as e:
