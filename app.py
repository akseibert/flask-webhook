...
def extract_fields_with_regex(text: str, chat_id: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if normalized_text.lower().startswith(("delete ", "remove ")):
            ...
            if direct_delete and "delete" in direct_delete:
                log_event("direct_delete_detected", input=text[:100], chat_id=chat_id)
...
# Replace your current function with this improved version
def powerful_delete_handler(text: str) -> Dict[str, Any]:
    """
    Robust delete handler that processes any 'Delete X' command 
    without requiring specific formatting
    """
    text = text.strip().lower()
    
    # Check if this is a delete command
    if not text.startswith(("delete ", "remove ")):
        return {}
    
    # Extract what to delete (everything after the first word)
    ...
    # If there's an explicit "from X" format, extract the category
    category = None
    if " from " in target:
        target_parts = target.split(" from ", 1)
        if len(target_parts) == 2:
            target = target_parts[0].strip()
            category = target_parts[1].strip()
            # Map category to internal field name
            category = FIELD_MAPPING.get(category, category)
    
    return {"delete": {"value": target, "category": category}}
...
FIELD_PATTERNS = {
    ...
    "delete": r'^(?:delete|remove)\s+([\w\s]+(?:\s+[A-Za-z]+)?)(?:\s+from\s+([\w\s]+))?(?:\s*(?:,|\.|$))',
...
if not result:
    ...
    # Extract site name - looks for "at the [site name]" or "site [name]"
    site_matches = re.findall(r'(?:at\s+the\s+|at\s+|site\s+)([A-Z][A-Za-z\s]+(?:Site|Building|Tower|Complex|Project)\s*[A-Z0-9]*)', text, re.IGNORECASE)
    if site_matches:
        result["site_name"] = site_matches[0].strip()
    
    # Extract segment - looks for "segment [number]" or "section [number]"
    segment_matches = re.findall(r'(?:segment|section)\s+([A-Z0-9]+)', text, re.IGNORECASE)
    if segment_matches:
        result["segment"] = segment_matches[0].strip()
    
    # Extract category - looks for "file this under [category]" or "category [name]"
    category_matches = re.findall(r'(?:file\s+this\s+under|category|kategorie)\s+([A-Za-z\u00C0-\u017F]+)', text, re.IGNORECASE)
    if category_matches:
        result["category"] = category_matches[0].strip()
    ...
    if company_matches:
        result["companies"] = [{"name": comp.strip()} for comp in company_matches]
    
    # Extract people - looks for names in context (capitalized names mentioned with actions)
    people_matches = re.findall(r'(?:and\s+)?([A-Z][a-z]+)\s+(?:handled|managed|supervised|was|did|took|worked)', text)
    if people_matches:
        result["people"] = [{"name": person.strip()} for person in people_matches]
    
    # Also look for "I was [doing something]" to capture the reporter
    if re.search(r'I\s+was\s+(\w+)', text, re.IGNORECASE):
        if "people" not in result:
            result["people"] = []
        # Don't add "I" as a person, but note that someone was supervising/reporting
    
    # Extract roles - looks for "Name handled/managed/was doing [role]" or "I was [role]"
    role_patterns = [
        r'([A-Z][a-z]+)\s+(?:handled|managed|operated|was\s+(?:the\s+)?)\s*(?:the\s+)?([^.]+?)(?:\.|,|$)',
        r'I\s+was\s+(?:the\s+)?([^.,]+?)(?:\.|,|and|$)',
        r'([A-Z][a-z]+)\s+(?:as\s+(?:the\s+)?|worked\s+as\s+(?:the\s+)?)([^.]+?)(?:\.|,|$)'
    ]
    
    roles_found = []
    for pattern in role_patterns:
        role_matches = re.findall(pattern, text, re.IGNORECASE)
        for match in role_matches:
            if isinstance(match, tuple) and len(match) == 2:
                person, role = match
                if person.lower() != "i":  # Skip "I was..." for person name
                    roles_found.append({"person": person.strip(), "role": role.strip()})
            elif isinstance(match, str):  # For "I was [role]" pattern
                roles_found.append({"person": "Reporter", "role": match.strip()})
    
    if roles_found:
        result["roles"] = roles_found
    
    # Extract services - looks for "services" or "provided" context
    service_matches = re.findall(r'(?:services?\s+(?:provided|included)|provided\s+services?)\s*:?\s*([^.]+)', text, re.IGNORECASE)
    if service_matches:
        services_text = service_matches[0]
        services = [s.strip() for s in re.split(r',|\s+and\s+', services_text) if s.strip()]
        result["services"] = [{"task": service} for service in services]
    
    # Extract tools - looks for "used [tool]" or "with [tool]"
    tool_patterns = [
        r'(?:used\s+(?:a\s+|an\s+)?)((?:[a-z]+\s+)*(?:crane|mixer|pump|hammer|drill|saw|excavator|bulldozer|loader)(?:\s+and\s+(?:a\s+|an\s+)?(?:[a-z]+\s+)*(?:crane|mixer|pump|hammer|drill|saw|excavator|bulldozer|loader))*)',
        r'(?:with\s+(?:a\s+|an\s+)?)((?:[a-z]+\s+)*(?:crane|mixer|pump|hammer|drill|saw|excavator|bulldozer|loader))'
    ]
    
    tools_found = []
    for pattern in tool_patterns:
        tool_matches = re.findall(pattern, text, re.IGNORECASE)
    ...
    if tools_found:
        result["tools"] = [{"item": tool} for tool in tools_found]
    
    # Extract activities - looks for "Tasks included [activities]" or "activities [list]"
    activity_patterns = [
        r'[Tt]asks?\s+included?\s+([^.]+)',
        r'[Aa]ctivit(?:y|ies)\s+(?:included?|covered?|were?)\s*:?\s*([^.]+)',
        r'(?:work|activities)\s+(?:included?|covered?)\s*:?\s*([^.]+)'
    ]
    
    activities_found = []
    for pattern in activity_patterns:
        activity_matches = re.findall(pattern, text)
        for match in activity_matches:
            activities_text = match.strip()
            activities = [a.strip() for a in re.split(r',|\s+and\s+', activities_text) if a.strip()]
            activities_found.extend(activities)
    
    if activities_found:
        result["activities"] = activities_found
    
    # Extract issues - looks for problem keywords
    issue_patterns = [
        r'(?:spotted|found|noticed|discovered)\s+(?:a\s+)?([^,]+(?:crack|leak|damage|issue|problem)[^.]*)',
        r'(?:issue|problem|delay|injury)\s*:?\s*([^.]+)',
        r'([^.]*(?:crack|leak|damage|broken|fault)[^.]*)'
    ]
    
    issues_found = []
    for pattern in issue_patterns:
        issue_matches = re.findall(pattern, text, re.IGNORECASE)
        for match in issue_matches:
            if match.strip() and len(match.strip()) > 3:  # Avoid very short matches
                has_photo = "photo" in match.lower() or "picture" in match.lower() or "took a photo" in text.lower()
                issues_found.append({"description": match.strip(), "has_photo": has_photo})
    
    if issues_found:
        result["issues"] = issues_found
    
    # Extract weather - looks for weather descriptions
    weather_patterns = [
        r'[Ww]eather\s+(?:was\s+|started\s+)?([^.]+?)(?:\.|then|,|$)',
        r'(?:sunny|cloudy|rainy|overcast|drizzle|snow|fog|windy)\s*(?:and\s+)?([^.]*?)(?:\.|,|$)',
        r'(?:started|began)\s+([^.]*(?:sunny|cloudy|rainy|overcast|drizzle|snow|fog|windy)[^.]*)'
    ]
    
    weather_found = []
    for pattern in weather_patterns:
        weather_matches = re.findall(pattern, text, re.IGNORECASE)
        weather_found.extend([w.strip() for w in weather_matches if w.strip()])
    
    if weather_found:
        # Combine all weather descriptions
        result["weather"] = " ".join(weather_found[:2])  # Take first 2 matches to avoid repetition
    
    # Extract time - looks for time references
    time_patterns = [
        r'(?:around\s+)?(?:time\s*:?\s*)?(morning|afternoon|evening|noon|midnight|\d{1,2}:\d{2}|\d{1,2}\s*(?:am|pm))',
        r'(?:all\s+day|full\s+day|entire\s+day|half\s+day)',
    ...
    for pattern in time_patterns:
        ...
        time_found.extend([t.strip() for t in time_matches if t.strip()])
    
    if time_found:
        result["time"] = time_found[0]  # Take first match
    
    # Extract impression - looks for overall assessment
    impression_patterns = [
        r'(?:impression|overall|assessment)\s*:?\s*([^.]+)',
        r'(?:everything(?:\s+is|\s+was)?|progress(?:\s+is|\s+was)?|status(?:\s+is|\s+was)?)\s+([^.]+)',
        r'(?:no\s+delays?|on\s+track|behind\s+schedule|ahead\s+of\s+schedule|smooth|difficult|challenging|productive)'
    ]
    
    impression_found = []
    for pattern in impression_patterns:
        impression_matches = re.findall(pattern, text, re.IGNORECASE)
        impression_found.extend([i.strip() for i in impression_matches if i.strip()])
    
    if impression_found:
        result["impression"] = impression_found[0]
    
    # Extract comments - looks for general notes or requests
    comment_patterns = [
        r'(?:please|note|comment|remark)\s*:?\s*([^.]+)',
        r'(?:thanks|thank\s+you)\s*[.!]?\s*$'  # Capture thank you as a comment indicator
    ]
    
    comments_found = []
    for pattern in comment_patterns:
        comment_matches = re.findall(pattern, text, re.IGNORECASE)
        comments_found.extend([c.strip() for c in comment_matches if c.strip()])
        ...
        if category:
            if category in SCALAR_FIELDS and result.get(category):
                ...
                if string_similarity(result[category].lower(), value_lower) >= 0.6:
                    ...
                    result[category] = ""
                    changes.append(f"cleared {category} '{value}'")
                ...
                # Handle companies
                for company in list(result["companies"]):
                    if isinstance(company, dict) and "name" in company:
                        if string_similarity(company["name"].lower(), value_lower) >= 0.6:
                ...
                # Handle activities
                for activity in list(result["activities"]):
            ...
            # If no category specified, search all fields for a match
            
            # Try companies first (most likely target)
            for company in list(result["companies"]):
                if isinstance(company, dict) and "name" in company:
                    if string_similarity(company["name"].lower(), value_lower) >= 0.6:
                        session_data[chat_id]["last_change_history"].append(("companies", existing_data["companies"].copy()))
        ...
        if category and value:  # Both category and value provided
            ...
            if category in SCALAR_FIELDS:
                ...
                if category == "people":
                    ...
                    # Remove company
                    removed = False
                    # First try exact match or high similarity
                    for i, company in enumerate(result["companies"]):
                        if isinstance(company, dict) and "name" in company:
                            company_name = company["name"].lower()
                            value_lower = value.lower()
                            if (company_name == value_lower or 
                                string_similarity(company_name, value_lower) >= CONFIG["NAME_SIMILARITY_THRESHOLD"] or
                                company_name in value_lower or value_lower in company_name):
                                del result["companies"][i]
                                changes.append(f"removed company '{company['name']}'")
                                removed = True
                                break
                    
                    # If no match found, try with lower threshold for company names
                    if not removed:
                        for i, company in enumerate(result["companies"]):
                            if isinstance(company, dict) and "name" in company:
                                company_name = company["name"].lower()
                                value_lower = value.lower()
                                if string_similarity(company_name, value_lower) >= 0.5:  # Lower threshold
                                    del result["companies"][i]
                                    changes.append(f"removed company '{company['name']}'")
                                    removed = True
                                    break
