...
# If no patterns matched, try natural language extraction
        if not result:
            ...
            if people_matches:
                result["people"] = [person.strip() for person in people_matches]
            ...
            for pattern in role_patterns:
                ...
                for match in role_matches:
                    if isinstance(match, tuple) and len(match) == 2:
                        person, role = match
                        if person.lower() != "i":
                            roles_found.append({"name": person.strip(), "role": role.strip()})
                    elif isinstance(match, str):
                        roles_found.append({"name": "Reporter", "role": match.strip()})
            ...
            # Extract activities - looks for "Tasks included [activities]"
            activity_patterns = [
            ...
            if weather_found:
                result["weather"] = " ".join(weather_found[:2])
