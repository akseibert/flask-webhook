...
def summarize_data(d):
    ...
    lines.append("🏢 **Companies**: " + ", ".join(c.get("name", "") for c in d.get("company", [])) or "")
    lines.append("👷 **People**: " + ", ".join(p for p in d.get("people", []) if p) or "")
    lines.append("🎭 **Roles**: " + ", ".join(f"{r.get('name', '')} ({r.get('role', '')})" for r in d.get("roles", [])) or "")
    lines.append("🔧 **Services**: " + ", ".join(s.get("task", "") for s in d.get("service", [])) or "")
    lines.append("🛠️ **Tools**: " + ", ".join(t.get("item", "") for t in d.get("tools", [])) or "")
    lines.append("📅 **Activities**: " + ", ".join(d.get("activities", [])) or "")
    lines.append("⚠️ **Issues**:")
    valid_issues = [i for i in d.get("issues", []) if isinstance(i, dict) and i.get("description", "").strip()]
    if valid_issues:
